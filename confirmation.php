...
$stmt_user = $conn->prepare("SELECT nom, prenom FROM utilisateur WHERE id = ?");
...
");
$stmt_commande->bind_param("ii", $commande_id, $user_id);
$stmt_commande->execute();
$commande = $stmt_commande->get_result()->fetch_assoc();

// 3. Récupération articles
$items = [];
if ($commande) {
    $stmt_items = $conn->prepare("
        SELECT ci.*, pr.nomp as product_name 
        FROM commande_items ci
        JOIN produits pr ON ci.produit_id = pr.idp 
        WHERE ci.commande_id = ?
    ");
    $stmt_items->bind_param("i", $commande_id);
    $stmt_items->execute();
...
<body>
    ...
    try {
        $db = new PDO(
            'mysql:servername=localhost;username = root;password = "";dbname = monsite;charset=utf8',
            'root',
            ''
        );
    ...
    <header class="header">
        ...
        <nav class="nav">
            <ul class="nav-list">
                ...
                <li class="nav-item icon-item">
                    <a href="bienve.php" class="nav-icon" aria-label="My account">
                        <!-- Modification ici : on pointe vers mycompte.php -->
    ...
    <main class="account-dashboard">
        <div class="sidebar">
            <section class="user-profile">
                ...
                <div class="user-greeting">
                    <p class="greeting">Hello, <strong><?= htmlspecialchars($nom . ' ' . $prenom) ?></strong></p>
            ...
            <nav class="account-menu">
                <div class="menu-section">
                    <a href="myordre.php" class="menu-item">
                ...
                <div class="menu-section">
                    <a href="logout.php" class="menu-item logout">
                        <!-- On crée un fichier logout.php pour déconnexion propre -->
        ...
        <div class="max">
            ...
            <div class="main-content">
                <h1>Merci pour votre commande, <?= htmlspecialchars($prenom) ?> !</h1>

                <div class="order-summary">
                    <h2>Récapitulatif</h2>
                    <p>N°: <strong>#<?= $commande['id'] ?? '' ?></strong></p>
                    <p>Date: <?= date('d/m/Y H:i', strtotime($commande['date_commande'] ?? 'now')) ?></p>
                    <p>Statut: <span class="status"><?= $commande['statut'] ?? 'en traitement' ?></span></p>
                </div>

                <div class="shipping-info">
                    <h3>Livraison</h3>
                    <p><?= htmlspecialchars("$prenom $nom") ?></p>
                    <p><?= htmlspecialchars($commande['adresse'] ?? '') ?></p>
                    <p><?= htmlspecialchars(($commande['code_postal'] ?? '') . ' ' . ($commande['ville'] ?? '')) ?></p>
                    <p>Tél: <?= htmlspecialchars($commande['telephone'] ?? '') ?></p>
                </div>

                <div class="payment-method">
                    <h3>Paiement</h3>
                    <p>
                        <?= match (strtolower($commande['payment_method'] ?? '')) {
                    ...
                    <p>Total: <?= number_format($commande['total'] ?? 0, 2, ',', ' ') ?> €</p>
                </div>
                ...
                <div class="order-items">
                    ...
                    <?php foreach ($items as $item): ?>
                        <div class="order-item">
                            <h4><?= htmlspecialchars($item['product_name'] ?? 'Produit inconnu') ?></h4>
                            <p>Quantité: <?= $item['quantite'] ?? 0 ?></p>
                            <p>Taille: <?= $item['taille'] ?? 'Non spécifiée' ?></p>
                            <p>Prix: <?= number_format($item['prix_unitaire'] ?? 0, 2, ',', ' ') ?> €</p>
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="next-steps">
                    ...
                    <p>Livraison prévue: <?= date('d/m/Y', strtotime('+3 days')) ?></p>
